...
class GMM:
    ...
    def _e_step(self, X: np.ndarray) -> np.ndarray:
        ...
        for k in range(self.n_components):
            try:
                ...
                det_cov = np.linalg.det(self.covs[k])

                if det_cov <= 0:
                    raise np.linalg.LinAlgError("Covariance matrix is not positive definite.")

                # Compute the Gaussian probability density for all samples for component k
                gamma[:, k] = self.pi[k] * self._gaussian(X, self.means[k], inv_cov, det_cov)
            except np.linalg.LinAlgError:
                # Handle singular covariance matrix by adding a small value to the diagonal
                print(f"Covariance matrix for component {k} is singular. Adding regularization.")
                self.covs[k] += 1e-6 * np.eye(D)
                inv_cov = np.linalg.inv(self.covs[k])
                det_cov = np.linalg.det(self.covs[k])
                gamma[:, k] = self.pi[k] * self._gaussian(X, self.means[k], inv_cov, det_cov)

    # Normalize the responsibilities across all components for each sample        
    ...
    def _m_step(self, X: np.ndarray, gamma: np.ndarray):
        ...
        n_soft += 1e-10
        self.pi = n_soft / N
        self.means = gamma.T @ X / n_soft[:, None] # [K, D]

        X_centered = X[:, np.newaxis, :] - self.means # [N, K, D]
        gamma_expanded = gamma[:, :, np.newaxis] # [N, K, 1]
        covs = np.einsum('nkd,nke->kde', X_centered, X_centered * gamma_expanded) / n_soft[:, np.newaxis, np.newaxis]
        
        covs += 1e-4 * np.eye(D)[None, :, :] # 防止奇异矩阵

        for k in range(self.n_components):
            if not np.all(np.linalg.eigvals(covs) > 0):
                print("Covariance matrix is not positive definite")
                covs += 1e-4 * np.eye(D)[None, :, :]

        assert np.all(np.linalg.eigvals(covs) > 0), "Covariance matrix is not positive definite"
...
def sample_from_gmm(gmm: GMM, pca: PCA, label: int, path: Union[str, Path]):
    ...
    sample_low_dim = np.random.multivariate_normal(mean, cov)
    sample_high_dim = pca.inverse_transform(sample_low_dim)
    sample_scaled = np.clip(sample_high_dim, 0, 255).astype(np.uint8)
    sample_image = sample_scaled.reshape(28, 28)
    # raise Exception("Not implemented yet")
    # END_YOUR_CODE
